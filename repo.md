...
* <a href="https://github.com/Azure/wvdquickstart/blob/main/ARMRunbookScripts/checkCredentials.ps1" target="_blank">checkAzureCredentials.ps1</a>: This script makes sure that the entered Azure Admin credentials are correct, in addition to validating the vnet, subnet, and firewall configuration. This script also configures the 'WVDServicePrincipal' managed identity in the deployment resource group to give it the *contributor* role on the subscription. This is needed to run deployment scripts in the main ARM template successfully.
